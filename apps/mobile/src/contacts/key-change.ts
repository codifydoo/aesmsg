...
// Pure key-change detection for the "this contact's key changed" MitM signal — the product's
// headline defense. Extracted (node-tested, no React / no storage) per the apps/mobile convention so
// the contact-detail re-scan / re-paste flow can DECIDE whether to raise the Key-Changed alert
// before committing anything to the store.
//
 ...
 *   - "rotated-back" → the candidate is a key this contact ALREADY rotated away from (the store
 *                      refuses to re-adopt it — RotatedAwayError).
 *   - "changed"      → a genuinely new key: raise the Key-Changed alert. Carries the REAL previous
 *                      (current-on-file) and new fingerprints for the alert's side-by-side compare.
 */
 ...
 * because fingerprinting is; throws if `candidatePublicKey` is not a valid key (the caller — the
 * paste screen — validates via importPublicKey first and maps any throw to inline copy).
 */
...
export interface KeyChangeAlertView {
  contactName: string;
  /** Previously-verified fingerprint — grey "Previous" cell. */
